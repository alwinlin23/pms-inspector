...
📄 **網站：** [alwinlin23.github.io/pms-inspector](https://alwinlin23.github.io/pms-inspector/)
...
## 一個設定的天壤之別 — Before / after

Claude Code 預設的 `skillListingBudgetFraction: 0.01` 幾乎必然把每個 skill 的描述壓縮到只剩 slug(名稱)。在 318 個 skill 的配置下,差別在於:模型看到的是 318 句它能**推理**的話,還是 318 個它只能**猜**的名字:

**Before —  `skillListingBudgetFraction: 0.01`  (Claude Code 預設值)**
...
est. cost:   ~ 18.9 k tokens / turn   (200k 視窗的 9.4 %)
```

差值約 **17k tokens / 輪** —— 花得非常值,因為正是這些 token 讓模型能判斷該調哪個 skill。

### 為什麼 "name-only" 是壞的 —— 人肉都判斷不了

下面就是當一個 skill 落到 `name-only` 時模型看到的東西。請你自己看這些 slug,問一下自己:*哪個該在什麼時候用?*
...
你判斷不了。Claude 也判斷不了。`smart-outline` 可能是從文件裡提取標題,也可能是**在寫程式前先梳理方案**。同一個 slug,兩種完全不同的觸發情境。乘以 318 之後,harness 乾脆不再路由到 skill —— 它把活直接在主 context 裡幹完,又貴又慢,你精心寫的每一個 skill 都跟沒裝一樣。

**描述**才是觸發句 —— 那句告訴模型*什麼時候該開火*的話。刪掉之後,slug 就只是一張羅夏墨跡圖。

修復只是 `~/.claude/settings.json` 裡的一行:

```jsonc
{
  "skillListingBudgetFraction": 0.15   // ← 原值 0.01 (CC 預設)
}
```

`/pms-inspector` 會算出適合你這套配置的確切數字,並透過 `--apply <plan-id>` 幫你寫進去,寫入前先做一份 `.bak.<ISO-時間戳>` 快照。
