...
📄 **ウェブサイト：** [alwinlin23.github.io/pms-inspector](https://alwinlin23.github.io/pms-inspector/)
...
## 一つの設定で天と地の差 — Before / after

Claude Code のデフォルト `skillListingBudgetFraction: 0.01` は、ほぼ確実にすべての skill の説明を slug(名前)だけに圧縮します。318 個の skill を積んだ構成では、モデルが**推論できる** 318 個の文を見るか、**当てずっぽうしかできない** 318 個の名前を見るかの違いになります:

**Before —  `skillListingBudgetFraction: 0.01`  (Claude Code デフォルト)**
...
est. cost:   ~ 18.9 k tokens / turn   (200k ウィンドウの 9.4 %)
```

差分はおよそ **17k tokens / ターン** —— 十分に見合う出費です。この tokens こそが、モデルにどの skill を起動すべきかを判断させるものだからです。

### なぜ "name-only" は壊れているか —— 人間でも判別できない

skill が `name-only` に落ちたとき、モデルが見ているのはこれです。以下の slug を眺めて、自分に問いかけてみてください: *どれを、いつ呼び出すべきか?*
...
判別できません。Claude にも判別できません。`smart-outline` は文書から見出しを抽出するものかもしれないし、**コードを書く前にプランを整理する**ものかもしれない。同じ slug、まったく別の起動条件です。それを 318 倍にすると、harness は skill にルーティングするのをやめてしまいます —— 仕事をぜんぶメインの context で片付けてしまい、コストは倍、しかも遅い。せっかく書いた skill はインストールされていないのと同じです。

**説明文**こそが起動のトリガー文 —— モデルに*いつ発火すべきか*を教える一文です。それを落とせば、slug はロールシャッハ・テストにしかなりません。

修正は `~/.claude/settings.json` の一行です:

```jsonc
{
  "skillListingBudgetFraction": 0.15   // ← 元は 0.01 (CC デフォルト)
}
```

`/pms-inspector` はあなたの構成にぴったりの数値を算出し、`--apply <plan-id>` で書き込みます。書き込み前に `.bak.<ISO タイムスタンプ>` のスナップショットが自動で取られます。
